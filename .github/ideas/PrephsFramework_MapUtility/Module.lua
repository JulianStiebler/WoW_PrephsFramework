...
local function LoadSettings()
    if not QuickMapSettings then
        QuickMapSettings = { scale = 0.8, zoom = 1.0, normX = 0.5, normY = 0.5, mapID = 0 }
    end
    WorldMapFrame:SetScale(QuickMapSettings.scale)
end
...
-- This function is the "Magic Fix" for the cursor offset
local function FinalHitboxSync()
    -- Recalculate the frame's internal size
    WorldMapFrame:OnFrameSizeChanged()
    
    if WorldMapFrame.ScrollContainer then
        -- Refresh the click-hitbox area
        WorldMapFrame.ScrollContainer:UpdateScrollChildRect()
    end
    
    -- Tells the engine to re-index the frame's position on the screen
    UpdateUIPanelPositions(WorldMapFrame)
end
-- 3. Create the Drag Handle (Top Bar)
local h = CreateFrame("Button", "QuickMapMoveHandle", WorldMapFrame)
h:SetPoint("TOPLEFT", WorldMapFrame.BorderFrame, "TOPLEFT", 20, 0)
h:SetPoint("BOTTOMRIGHT", WorldMapFrame.BorderFrame, "TOPRIGHT", -50, -25)
...
h:SetScript("OnDragStop", function()
    ...
    -- Sync coordinates to a clean TOPLEFT anchor
    local s = WorldMapFrame:GetScale()
    local left = WorldMapFrame:GetLeft()
    local top = WorldMapFrame:GetTop()
    
    WorldMapFrame:ClearAllPoints()
    WorldMapFrame:SetPoint("TOPLEFT", UIParent, "BOTTOMLEFT", left, top)
    
    -- This is the magic bullet for the click-offset
    FinalHitboxSync()
end)
-- 4. Zoom & Pan Logic
...
local function UpdatePanState()
    local base = sc.baseScale or 1
    local curZoom = sc.targetScale or base
    sc.shouldPanOnClick = (curZoom > (base + 0.05))
end

sc:SetScript("OnMouseWheel", function(self, delta)
    local base = self.baseScale or 1
    local curZoom = self.targetScale or base
    local zoomStep = 0.1
    local nextZoom = (delta > 0) and (curZoom + zoomStep) or (curZoom - zoomStep)

    nextZoom = math.max(base, math.min(2.5, nextZoom))
    self.targetScale = nextZoom
    
    UpdatePanState()
    
    if QuickMapSettings then QuickMapSettings.zoom = nextZoom end
    if self.ZoomToNextSetScale then self:ZoomToNextSetScale() end
end)

-- 5. Deep Sync on Map Change (Right-Click Fix)
hooksecurefunc(WorldMapFrame, "OnMapChanged", function()
    -- Reset custom zoom state so we don't start 'zoomed in' on the continent
    sc.targetScale = sc.baseScale or 1
    sc.shouldPanOnClick = false
    C_Timer.After(0.01, FinalHitboxSync)
end)
-- 6. Escape Key Fix
tinsert(UISpecialFrames, "WorldMapFrame")

-- 7. Resize Handle (Simpler Math)
local function CreateResizeHandle(point)
    local rb = CreateFrame("Button", nil, WorldMapFrame)
    rb:SetSize(20, 20)
    if point == "BOTTOMLEFT" then rb:SetPoint("BOTTOMLEFT", WorldMapFrame, "BOTTOMLEFT", 18, 8)
    else rb:SetPoint("BOTTOMRIGHT", WorldMapFrame, "BOTTOMRIGHT", -8, 8) end
    
    local function SetupTex(texPath)
        local tex = rb:CreateTexture(nil, "OVERLAY")
        tex:SetTexture(texPath)
        tex:SetAllPoints()
        tex:SetVertexColor(1, 1, 0, 1)
        if point == "BOTTOMLEFT" then tex:SetTexCoord(1, 0, 0, 1) end
        return tex
    end

    rb:SetNormalTexture(SetupTex("Interface\\ChatFrame\\UI-ChatIM-SizeGrabber-Up"))
    rb:SetPushedTexture(SetupTex("Interface\\ChatFrame\\UI-ChatIM-SizeGrabber-Down"))
    rb:SetHighlightTexture(SetupTex("Interface\\ChatFrame\\UI-ChatIM-SizeGrabber-Highlight"))
    rb:RegisterForDrag("LeftButton")

    rb:SetScript("OnDragStart", function(self)
        local startScale = WorldMapFrame:GetScale()
        local uiScale = UIParent:GetEffectiveScale()
        local _, topY = WorldMapFrame:GetCenter()
        topY = WorldMapFrame:GetTop() -- Anchor to top during resize

        self:SetScript("OnUpdate", function()
            local _, cursorY = GetCursorPosition()
            local currentTop = topY * startScale
            local newScale = math.abs(currentTop - cursorY) / (WorldMapFrame:GetHeight() * uiScale)
            newScale = math.max(0.3, math.min(1.5, newScale))
            WorldMapFrame:SetScale(newScale)
        end)
    end)

    rb:SetScript("OnDragStop", function(self)
        self:SetScript("OnUpdate", nil)
        FinalHitboxSync()
        if QuickMapSettings then QuickMapSettings.scale = WorldMapFrame:GetScale() end
    end)
end

CreateResizeHandle("BOTTOMLEFT")
CreateResizeHandle("BOTTOMRIGHT")

-- 8. Final Refresh on Show
WorldMapFrame:HookScript("OnShow", function()
    if QuickMapSettings then
        WorldMapFrame:SetScale(QuickMapSettings.scale)
        FinalHitboxSync()
        
        if WorldMapFrame:GetMapID() == QuickMapSettings.mapID then
            C_Timer.After(0.05, function()
                if QuickMapSettings.zoom and QuickMapSettings.normX then
                    sc:InstantPanAndZoom(QuickMapSettings.zoom, QuickMapSettings.normX, QuickMapSettings.normY)
                    sc.targetScale = QuickMapSettings.zoom
                    UpdatePanState()
                end
            end)
        else
...
-- 9. Map Reveal Logic (Corrected ArtID Lookup)
local function MapExplorationPin_RefreshOverlays(pin)
    local mapID = WorldMapFrame.mapID
    if not mapID then return end

    local artID = C_Map.GetMapArtID(mapID)
    if not artID or not RevealData[artID] then return end

    -- 1. Identify explored tiles
    local exploredTiles = {}
    local exploredMapTextures = C_MapExplorationInfo.GetExploredMapTextures(mapID)
    if exploredMapTextures then
        for _, info in ipairs(exploredMapTextures) do
            -- We store the key to avoid over-drawing what you already know
    ...
    local layerIndex = pin:GetMap():GetCanvasContainer():GetCurrentLayerIndex()
    local layerInfo = layers and layers[layerIndex]
    if not layerInfo then return end
    
    local TILE_W, TILE_H = layerInfo.tileWidth, layerInfo.tileHeight

    -- 3. Draw the "Ghost" textures
    for key, files in pairs(RevealData[artID]) do
        if not exploredTiles[key] then
            local width, height, offX, offY = strsplit(":", key)
            width, height = tonumber(width), tonumber(height)
            offX, offY = tonumber(offX), tonumber(offY)
            
            local fileDataIDs = { strsplit(",", files) }
            local numW, numH = math.ceil(width/TILE_W), math.ceil(height/TILE_H)

            for j = 1, numH do
                for k = 1, numW do
                    local texture = pin.overlayTexturePool:Acquire()
                    
                    local pixH = (j < numH) and TILE_H or (height % TILE_H == 0 and TILE_H or height % TILE_H)
                    local pixW = (k < numW) and TILE_W or (width % TILE_W == 0 and TILE_W or width % TILE_W)
                    
                    local fileW, fileH = 16, 16
                    while fileW < pixW do fileW = fileW * 2 end
                    while fileH < pixH do fileH = fileH * 2 end

                    texture:SetSize(pixW, pixH)
                    texture:SetTexCoord(0, pixW/fileW, 0, pixH/fileH)
                    texture:SetPoint("TOPLEFT", offX + (TILE_W * (k-1)), -(offY + (TILE_H * (j-1))))
                    
                    local index = ((j-1) * numW) + k
                    local fileID = tonumber(fileDataIDs[index])
                    ...
                    if fileID then
                        ...
                        -- 1.0, 1.0, 1.0, 0.4 = Full color but faded
                        -- 0.5, 0.5, 0.5, 0.5 = Grey and semi-transparent
                        texture:SetVertexColor(0.7, 0.7, 0.7, 0.6) 
                        
                        -- Set to BACKGROUND to ensure it stays under player-explored icons
...
-- 10. Hooking into the Pin system
local function SetupMapHooks()
    for pin in WorldMapFrame:EnumeratePinsByTemplate("MapExplorationPinTemplate") do
        if not pin.isHookedReveal then
            hooksecurefunc(pin, "RefreshOverlays", MapExplorationPin_RefreshOverlays)
            
            pin.overlayTexturePool.resetterFunc = function(pool, tex)
                tex:SetVertexColor(1, 1, 1, 1)
                tex:SetAlpha(1)
                tex:Hide()
                tex:ClearAllPoints()
            end
            pin.isHookedReveal = true
        end
    end
end

-- Ensure it runs when the map is ready
if WorldMapFrame.initialized then 
    SetupMapHooks() 
else 
    hooksecurefunc(WorldMapFrame, "OnMapChanged", SetupMapHooks)
end
